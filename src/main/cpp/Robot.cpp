...
//anywho i suppose ill head out

//i completey missed the last message but ok bye
...
rev::SparkRelativeEncoder m_LeftShooterMotorEncoder{ m_LeftShooterMotor.GetEncoder() };//formerly passed rev::SparkRelativeEncoder::Type::kHallSensor to GetEncoder
rev::SparkRelativeEncoder m_RightShooterMotorEncoder{ m_RightShooterMotor.GetEncoder() };//formerly passed rev::SparkRelativeEncoder::Type::kHallSensor to GetEncoder
...
//static const nt::NetworkTableEntry doYouLikeMe = frc::Shuffleboard::GetTab("y");
...
void MotorInitVictor( WPI_VictorSPX &m_motor )
 ...
 public:
  ...
  void TestPeriodic() override {
    ...
    //m_swerve.Drive(, units::velocity::feet_per_second_t{0}, units::angular_velocity::radians_per_second_t{0}, false, m_driverController.GetBButton());
 ...
 private:
  ...
  void DriveWithJoystick(bool fieldRelative) {
    // std::cout << Drivetrain::m_frontLeft.GetPosition().angle.Degrees().value() +
    //              Drivetrain::m_frontRight.GetPosition().angle.Degrees().value() +
    //              Drivetrain::m_backLeft.GetPosition().angle.Degrees().value() +
    //              Drivetrain::m_backRight.GetPosition().angle.Degrees().value();
  ...
  //ySpeed = units::velocity::feet_per_second_t{0};
  //xSpeed = units::velocity::feet_per_second_t{0};
...
//    }
  ...
  */
  void ExecutiveImportGradleSourceDirective() {
    ...
    frc::SmartDashboard::PutString("Spash", splashes[randMessage]);
    // std::cout << MAGENTA << splashes[randMessage] << BLANK
    //           << std::endl;
    
    //frc::ReportError();
    //FRC_ReportError(0, "Hello world!");
    return;
  }
